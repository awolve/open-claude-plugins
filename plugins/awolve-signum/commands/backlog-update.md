...
- To reparent an item under a different epic, use `/awolve-signum:backlog-set-parent` — clearer error reporting and validation.
...
- **Applying a tag needs only the right to edit the item** — it is the tag *vocabulary* that is permission-gated, not its use. See `/awolve-signum:tags` for the project's list.
- To remove an item entirely, use `/awolve-signum:backlog-delete` (soft-delete).
