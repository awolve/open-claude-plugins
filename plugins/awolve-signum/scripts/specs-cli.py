...
def resolve_doc_id(file_path):
    ...
    if not headers:
        print("specs: not authenticated — run /awolve-signum:login first", file=sys.stderr)
        sys.exit(1)
...
def pull_project(
    ...
    if status == 401:
        if not quiet:
            print("specs: authentication expired — run /awolve-signum:login", file=sys.stderr)
        return report
    if status == 404:
        if not quiet:
...
def feature_snapshot(project_id, feature_name, as_json=False):
    ...
    if status_code in (401, 403):
        print("specs: authentication failed — run /awolve-signum:login", file=sys.stderr)
        sys.exit(1)
    if status_code == 404:
        print(f"specs: feature '{feature_id}' not found", file=sys.stderr)
        sys.exit(1)
...
def attach_file(file_path, feature_identifier=None):
    ...
    cfg = config.read_config()
    if not cfg:
        print("specs: no config found — run /awolve-signum:login", file=sys.stderr)
        sys.exit(1)

    headers = auth.get_headers()
    if not headers:
        print("specs: not authenticated — run /awolve-signum:login", file=sys.stderr)
        sys.exit(1)
