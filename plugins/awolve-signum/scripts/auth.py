...
def get_headers():
    ...
    if method == "azure-cli":
        ...
        if not token:
            print("specs: Azure CLI token unavailable — run 'az login' or '/awolve-signum:login'", file=sys.stderr)
            return None
...
def login_azure(service_url=None):
    ...
    if not _verify_token(token, url):
        ...
        print(f"specs: use an API key instead: /awolve-signum:login with --api-key", file=sys.stderr)
        return False
...
def status():
    ...
    if method == "azure-cli":
        ...
        print("specs: not authenticated — run /awolve-signum:login")
