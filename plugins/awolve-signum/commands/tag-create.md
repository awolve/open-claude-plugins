...
**Look before you create.** Run `/awolve-signum:tags` first, or read the suggestions the service returns — most "new" tags are a re-spelling of one that already exists, and a split vocabulary is worse than a slightly wrong name.
...
- To apply a tag once it exists: `/awolve-signum:backlog-update --add-tag` or `/awolve-signum:update-bug --add-tag`, or the picker on any item in the portal.
