...
When the last task is checked off, run the closure checklist in the `awolve-signum:spec` skill (docs match reality → shipped per the repo's ship cycle → statuses flipped → living docs updated).
