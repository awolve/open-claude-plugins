...
# /awolve-spec:feedback-key-revoke
...
Parse `<project> <user-ref> <key>` where `<key>` is the key id, a prefix of the id, or the key prefix shown by `/awolve-spec:feedback-users` (`sk_ab12cd…`).
...
1. `/awolve-spec:feedback-key-create` — mint the replacement.
2. Put the new key in the app's secret store and roll the app.
3. `/awolve-spec:feedback-key-revoke` — kill the old one.
