...
# /awolve-signum:edit-bug-comment
...
Bug references accept the short numeric form (with or without `#`). The comment ID is the UUID shown in brackets by `/awolve-signum:bug-comments`. Quote the body if it contains spaces.
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/specs-cli.py edit-bug-comment spec-service 15 7f5d… "Shipped in awolve-signum v0.16.3 — commit abc1234."
```

## Notes

- Use `/awolve-signum:bug-comments <project> <bug-number>` first to find the comment ID.
...
- The edit is logged with the bug's project as the audit scope, so it shows up in `/awolve-signum:log` for that project.
