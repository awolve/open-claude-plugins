...
Run this after `/awolve-signum:design` when the feature touches cloud infrastructure, new services, networking, deployment, or system topology. Skip it for pure application-code changes with no infra footprint.
...
Find the feature folder. It must have a `design.md` — if it doesn't, tell the user to run `/awolve-signum:design` first.
...
Next step: run `/awolve-signum:plan` to create the implementation plan, or get the design reviewed first.
```
