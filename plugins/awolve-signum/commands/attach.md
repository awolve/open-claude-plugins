...
# /awolve-signum:attach

Upload a local binary file (image, PDF, Excel, etc.) as an attachment. Three target entities are supported: **feature** (default), **bug**, and **backlog item**. The file is stored in Azure Blob Storage via Signum. Feature attachments are mirrored back to every team member's local feature folder on the next `/awolve-signum:pull`; bug + backlog attachments live only in the portal/server.
...
- Feature attachments sync to disk on the next `/awolve-signum:pull`; bug/backlog attachments do not
- Use `/awolve-signum:list-attachments`, `/awolve-signum:download-attachment`, and `/awolve-signum:delete-attachment` to inspect or remove attachments after upload
