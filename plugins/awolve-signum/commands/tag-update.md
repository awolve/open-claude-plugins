...
- Renaming into a name that is already taken fails with `tag_slug_taken` — there is no merge in this version. To consolidate two tags by hand: retag the items off the loser (`--remove-tag old --add-tag keeper`), then `/awolve-signum:tag-delete` the empty one.
