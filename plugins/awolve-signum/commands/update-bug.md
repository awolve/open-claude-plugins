...
- For status changes use `/awolve-signum:set-bug-status` — it emits a dedicated `status_change` audit event.
...
- To attach resolution context (commit SHA, rollout notes) without rewriting the description, prefer `/awolve-signum:bug-comment` so the original report stays intact.
