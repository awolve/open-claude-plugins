...
- **Tags** (optional) — if the user labels it ("this is a billing thing"), pass `--tags billing` (comma-separated for several). The tags must already exist on the project; run `/awolve-signum:tags` to see them and `/awolve-signum:tag-create` to coin one first.
...
`--assignee` takes an email. The person must be able to see the project — internal Awolve users always can; external users need project access first, or the create fails with `assignee_no_access`. Assignment can also be added later with `/awolve-signum:backlog-update --assignee`.
...
Confirm the item was created. Mention that `/awolve-signum:backlog-update --status` flips its status as it moves through the workflow, and that if it grows into something that needs a spec, `/awolve-signum:req <project-id> <item-number>` creates a feature from it and links the item as the one that delivers it. To link it to a feature that already exists: `/awolve-signum:backlog-update <project-id> <item-number> --feature <feature-name>`.
