...
Find the specs path and feature folder. If the feature folder doesn't exist, determine the next spec number and create it (same as `/awolve-signum:req`).
...
Next steps:
- If this feature touches infrastructure, run `/awolve-signum:infra` to detail the infra changes
- Otherwise, run `/awolve-signum:plan` to create the implementation plan
